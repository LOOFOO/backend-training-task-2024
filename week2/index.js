...
var l = '8';
// 請文字解釋為什麼
console.log(k * l > 21); // 乘法時js判斷l字串可轉成數字，就會自動把l轉成數字與k相乘
...
const gymCoach = {
	WangCoach : {
		Specialist : ['strengthTraining', 'weightLossProgram'],
		Programs :[
		...
		backgroundIntroduction : '王教練擁有 5 年教學經驗，專精於提升學員的肌力與減脂，適合希望快速達成體能目標的學員。',
		acceptanceNewStudents : true
	},
	LeeCoach : {
		Specialist : ['yoga', 'bodySculpting'],
		Programs :[
									personalTrainingCourses =	{
											 persession : 1800, 
											 coursesMinute : 50, 
		...
		backgroundIntroduction : '李教練是一位瑜伽大師，擁有 10 年教學經驗，擅長幫助學員雕塑完美體態，適合希望改善姿態與柔軟度的學員。',
		acceptanceNewStudents : false
	}	
};
...
// 練習：計算小明今日消耗的卡路里
calorieBurn += activities.treadmill.minuteCalorie * 30;
calorieBurn += activities.yoga.minuteCalorie * 40;
calorieBurn += activities.bicycling.minuteCalorie * 10;
...
// 練習：週二、週三、週四、週五、週六
if (exerciseRecords[1].duration >= 30 && exerciseRecords[0].heartRate >= 130) {
  totalDuration += exerciseRecords[1].duration;
  validDays += 1;
}
if (exerciseRecords[2].duration >= 30 && exerciseRecords[0].heartRate >= 130) {
  totalDuration += exerciseRecords[2].duration;
  validDays += 1;
}
if (exerciseRecords[3].duration >= 30 && exerciseRecords[0].heartRate >= 130) {
  totalDuration += exerciseRecords[3].duration;
  validDays += 1;
}
if (exerciseRecords[4].duration >= 30 && exerciseRecords[0].heartRate >= 130) {
